...
// The screenshot helpers are backend-agnostic: they capture via the CDP client
// every Session holds, then reuse transient visual evidence or hand the image to
// the configured vision model. `take_screenshot` inspects pixels only; `save_image`
// returns bytes so the loop can persist a report asset.
//
...
async function saveImage({ session, ref, brief, signal } = {}) {
...
async function screenshot(args = {}) {
  const out = await saveImage(args);
  const { image, ...detail } = out;
  return detail;
}

module.exports = { screenshot, saveImage, capture, clipForRef, imageEncoding };
