...
recordContract MUST be null.`
      : `The operator set mode=auto. Choose taskType by what the DELIVERABLE is, not
whether a number appears:
  - "records" when the deliverable IS a collected set of similar items read off
    pages and handed back as a list/ledger — e.g. jobs, contacts, vendors, grants,
    leads, papers, examples. The work is finding and capturing data about each item.
  - "research" when the task asks for investigation, comparison, explanation,
    recommendation, or synthesis without a clear final-record count — AND for
    action tasks where the real work is DOING something to each item (apply to,
    fill out, submit, message, book, purchase, draft-and-send) rather than
    collecting data about it. A count there ("apply to 5 jobs") is a stop
    condition that belongs in requirements, not a reason to build a record ledger.
    Only choose "records" for such a task if the operator explicitly asks for a
    returned list/log of the items as the deliverable.`;
