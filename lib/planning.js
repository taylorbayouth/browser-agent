...
This is a north star to keep the run aligned, not a rigid script: the live page
is always ground truth and you may adapt as you learn. Commit only to what you
can reasonably know now — do NOT invent specific sites, prices, names, dates, or
findings you cannot yet know; describe the shape of the work, not imagined results.
...
async function generatePlan({ task, context, provider, model, reasoningEffort, maxTokens, timeoutMs }) {
  const message = buildPlanMessage({ task, context });
  const completion = await callModel(
    { system: PLAN_SYSTEM, tools: [], messages: [message], model, reasoningEffort, maxTokens, timeoutMs },
    { provider }
  );
