  ...
  for (const doc of snapshot.documents || []) {
    ...
    for (let i = 0; i < nodeName.length; i++) {
      ...
      regions.push({
        ...
        role: REGION_ROLE[tag], bbox, inViewport, backendNodeId: backendIds[i],
        named, label, area,
        dedupeKey: (tag === 'IMG' || tag === 'VIDEO' || tag === 'EMBED') ? (attrs.src || '')
...
async function performExtract(client, opts = {}) {
  ...
  for (let i = 0; i < regions.length; i++) {
    ...
    regions[i] = { ref, role: regions[i].role, bbox: regions[i].bbox, inViewport: regions[i].inViewport, named: regions[i].named, label: regions[i].label };
  }
