...
const STYLE_PROPS = [
  ...
  'background-repeat', 'background-size', 'color',
  'font-size', 'font-weight', 'border-radius', 'overflow',
];

// Tags whose rendered content the accessibility tree cannot describe in text: a
// <canvas> is raw pixels, an <img> is a bitmap, an <svg> is vector paint, a
// <video> shows a poster frame or live pixels, and <object>/<embed> may render
// an image or opaque plugin content. We surface rendered graphics as @v "visuals"
// the planner can inspect or save them —
// both the UNNAMED ones (no accessible name; the AX listing is silent and the
// agent is blind — capture to READ) and the NAMED ones (alt/aria/<title>: logos,
// photos, product shots — capture to SHOW in the report). This is a deterministic
// structural fact ("a graphic of this size is rendered here"), NOT a heuristic
// guess about importance — we report, the model decides. nodeName is uppercase
// for HTML (CANVAS/IMG) and lowercase for the XML-cased SVG element, so all
// comparisons uppercase first.
//
...
const REGION_ROLE = { CANVAS: 'canvas', IMG: 'image', SVG: 'graphic', IFRAME: 'iframe', VIDEO: 'video', OBJECT: 'object', EMBED: 'object' };

// A NAMED visual (logo/photo) below this area (~80×80) is an avatar or icon, not
// content worth showing — drop it. Unnamed graphics are never floored (the model
// is blind to them); both still need a non-zero rendered box.
const MIN_VISUAL_AREA = 6400;
// Cap named visuals per brief so a gallery or avatar list can't flood perception.
// Unnamed graphics are never capped — the model can't see them any other way.
const NAMED_VISUAL_CAP = 10;
...
// AX `invalid` is a TOKEN, not a boolean: "false" (the overwhelmingly common
// case) | "true" | "grammar" | "spelling". The "false" string is itself truthy,
// so a naive `if (props.invalid)` would flag every field — test against it
// explicitly. Any non-false token means the control failed validation, which is
// a direct, reliable signal for form-filling (far better than hoping a nearby
// error string happened to surface as a @t node).
function isInvalid(v) {
  return v != null && v !== false && v !== 'false';
}

// AX `haspopup` is also a token: "false" (no popup) | "true" | "menu" |
// "listbox" | "tree" | "grid" | "dialog". Same "false"-is-a-truthy-string trap
// as isInvalid. Returns the popup kind to surface — so the model knows a click
// opens something, and what — or null when the element opens nothing.
function popupKind(v) {
  if (v == null || v === false || v === 'false') return null;
  return v === true ? 'true' : String(v);
}

// A password field's value must never reach the model. The AX tree usually
// leaves it empty already, but we redact defensively in case a build echoes it.
// We keep a fixed-length placeholder when the field is non-empty so the model
// still knows it was filled (and need not re-type) without seeing the secret —
// and the fixed length never leaks how long the password is.
function safeValue(node, isPassword) {
...
function cssBackgroundUrl(style, baseUrl) {
  const raw = firstCssUrl(style?.['background-image']);
  return raw ? absoluteHttpUrl(raw, baseUrl) : null;
}

function isRepeatingBackground(style) {
  const repeat = String(style?.['background-repeat'] || '').toLowerCase();
  return repeat.includes('repeat') && !repeat.includes('no-repeat');
}

// Classify a rendered graphic for the @v listing. Mirrors how the platform names
// these elements:
//   - aria-label / aria-labelledby name any element.
//   - <img>: an `alt` attribute names it; alt="" is the author marking it
//     decorative ("skip me"); a *missing* alt is an unnamed image.
//   - <svg>: a child <title> element is its accessible name.
//   - <video>: a `poster` attribute means there's a visible preview image to SHOW.
//     Without poster it's opaque pixels — capture to READ.
//   - <object>/<embed>: `type` starting with "image/" means it's an image; otherwise
//     treat as unnamed opaque content — capture to READ.
// Returns either:
//   { decorative: true }  — drop it (author marked it skippable), or
//   { named, label }      — named===true → capture to SHOW (logo/photo); false →
//                           unreadable, capture to READ. `label` is a human string
//                           for the listing (name or title tooltip), '' if none.
function classifyGraphic(tag, attrs, hasTitleChild) {
  const hasAria = !!(attrs['aria-label']?.trim() || attrs['aria-labelledby']?.trim());
  if (!hasAria && tag === 'IMG' && 'alt' in attrs && !(attrs.alt || '').trim()) {
    return { decorative: true };
  }
  const named = hasAria
    || (tag === 'IMG' && 'alt' in attrs)
    || (tag === 'SVG' && hasTitleChild)
    || (tag === 'VIDEO' && !!attrs.poster)
    || ((tag === 'OBJECT' || tag === 'EMBED') && (attrs.type || '').startsWith('image/'));
  const label = (attrs['aria-label'] || attrs['alt'] || attrs['title'] || '').trim();
  return { named, label };
}
...
// backendNodeIds of every <input type="password"> in the snapshot, so the
// element builders can redact their values out of the brief. One cheap pass over
// the same node tables collectRegions walks; reuses parseSnapshotAttrs to read
// the type attribute.
function collectPasswordIds(snapshot) {
  const ids = new Set();
  ...
  for (const doc of snapshot.documents || []) {
    ...
    for (let i = 0; i < nodeName.length; i++) {
      ...
      if ((attrs.type || '').toLowerCase() === 'password') ids.add(backendIds[i]);
    }
  }
  return ids;
}
...
function collectRegions(snapshot, maps, viewport, opts) {
  ...
  for (const doc of snapshot.documents || []) {
    ...
    for (let i = 0; i < nodeName.length; i++) {
      ...
      const style = !isGraphic && !isFrame ? getComputedStyles(backendIds[i], maps) : null;
      const backgroundUrl = style ? cssBackgroundUrl(style, docUrl) : null;
      const isCssBackground = !!backgroundUrl;
      if (!isGraphic && !isFrame && !isCssBackground) continue;

      const attrs = parseSnapshotAttrs(attributes[i], strings);
      if (attrs['aria-hidden'] === 'true') continue;            // author hid it from a11y → decorative
      const rawSource = tag === 'IMG' || tag === 'EMBED' ? attrs.src
        : tag === 'VIDEO' ? (attrs.poster || attrs.src)
        : tag === 'OBJECT' ? attrs.data
        : '';
      const sourceUrl = backgroundUrl || absoluteHttpUrl(rawSource, docUrl);

      let named, label;
      if (isCssBackground) {
        if (isRepeatingBackground(style)) continue;
        named = false;
        label = '';
      } else if (isGraphic) {
        const cls = classifyGraphic(tag, attrs, titleParents.has(i));
        if (cls.decorative) continue;                           // img alt="" → author said skip
        ({ named, label } = cls);
      } else {
        if (embeddedFrames.has(i)) continue;   // same-process iframe — content already in the brief
        // A cross-origin iframe's pixels are always opaque to us, titled or not —
        // capture to READ. Any title/aria-label is only a display label.
        named = false;
        label = (attrs['aria-label'] || attrs.title || '').trim();
      }
      ...
      if (isCssBackground && area < MIN_VISUAL_AREA) continue;
      if (named && area < MIN_VISUAL_AREA) continue;            // tiny named graphic = avatar/icon
      ...
      // to crop a screenshot to this region); it's stripped before the region lands
      // in the brief, like elements/text never expose their node id. order/area/
      // dedupeKey let the caller dedupe + cap named visuals biggest-first while
      // preserving document (reading) order.
      regions.push({
        order: regions.length,
        role: isCssBackground ? 'background-image' : REGION_ROLE[tag], bbox, inViewport, backendNodeId: backendIds[i],
        named, label, area,
        sourceUrl: sourceUrl || null,
        dedupeKey: isCssBackground ? backgroundUrl : (sourceUrl || rawSource || ''),
      });
    }
  }
  return regions;
...
function buildTreeInteractive(node, props, bbox, inViewport, ref, isPassword) {
  const out = { ref, role: node.role?.value };
  const name = node.name?.value;
  if (name) out.name = name;
  out.bbox = bboxArr(bbox);
  if (inViewport) out.inViewport = true;
  const value = safeValue(node, isPassword);
  ...
  const popup = popupKind(props.haspopup);
  if (popup) out.haspopup = popup;
  if (props.required === true) out.required = true;
  if (isInvalid(props.invalid)) out.invalid = true;
  ...
  function collect(nodeId) {
    ...
    if (!node.ignored) {
      ...
      if (hasRole || isCustomWidget || cursorClickable) {
        ...
        lookup[ref] = id;
        return [buildTreeInteractive(node, props, bbox, inViewport, ref, maps.passwordIds?.has(id))];
      }
...
function buildLeanElement(node, props, ref, bbox, inViewport, isPassword) {
...
async function performExtract(client, opts = {}) {
  ...
  // Defaults to 1 on a 1:1 display; dump it with BROWSER_AGENT_DEBUG_COORDS=1 to confirm
  // on a Retina/scaled display.
  maps.scale = deriveDeviceScale(layoutMetrics);
  // backendNodeIds of password inputs, so the element builders can redact their
  // values out of the brief — a secret must never reach the model.
  maps.passwordIds = collectPasswordIds(snapshotResult);
  if (process.env.BROWSER_AGENT_DEBUG_COORDS) console.error(`[extract] bounds scale = ${maps.scale}`);
  ...
  for (const { node, props, source } of interactiveNodes) {
    ...
    if (format === 'lean') {
      elements.push(buildLeanElement(node, props, ref, bbox, inViewport, maps.passwordIds?.has(id)));
    } else {
      elements.push({
        ref,
  ...
  // Rendered graphics the listing above can't speak to in text. Each gets a @v
  // ref + lookup entry so the model can take_screenshot/save_image(@vN) and have
  // the capture cropped to the graphic's bbox. Only visual actions accept a @v ref —
  // click/type/select_text reject it. Unnamed graphics (charts, maps, scans,
  // CAPTCHAs) are captured to READ; named ones (logos, photos) to SHOW.
  //
  // Keep every unnamed graphic — the model is blind to those — but cap the named
  // visuals so a gallery/avatar list can't flood the brief: dedupe by source, keep
  // the biggest NAMED_VISUAL_CAP, then restore document (reading) order for refs.
  const collected = collectRegions(snapshotResult, maps, viewport, opts);
  const unnamed = collected.filter(r => !r.named);
  ...
  for (const r of collected) {
    ...
    if (r.dedupeKey) {
      ...
      seenKeys.add(r.dedupeKey);
    }
    named.push(r);
  }
  named.sort((a, b) => b.area - a.area);   // biggest first
  const regions = [...unnamed, ...named.slice(0, NAMED_VISUAL_CAP)]
    .sort((a, b) => a.order - b.order);     // restore reading order for refs
  for (let i = 0; i < regions.length; i++) {
    ...
    regions[i] = {
      ...
      role: regions[i].role,
      bbox: regions[i].bbox,
      inViewport: regions[i].inViewport,
      named: regions[i].named,
      label: regions[i].label,
      sourceUrl: regions[i].sourceUrl || undefined,
    };
  }
...
module.exports = {
  ...
  flattenProperties, isInViewport, isInPerceptionBand, isLeanVisible, isCursorClickable, isInvalid, popupKind, safeValue, bboxArr,
  collectRegions, collectPasswordIds, parseSnapshotAttrs, classifyGraphic, buildSnapshotMaps,
