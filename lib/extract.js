...
// <canvas> is raw pixels, an <img> is a bitmap, an <svg> is vector paint. We
// surface rendered graphics as @v "visuals" the planner can take_screenshot —
...
function classifyGraphic(tag, attrs, hasTitleChild) {
  ...
  const named = hasAria || (tag === 'IMG' && 'alt' in attrs) || (tag === 'SVG' && hasTitleChild);
  ...
  for (const doc of snapshot.documents || []) {
    ...
    for (let i = 0; i < nodeName.length; i++) {
      ...
      regions.push({
        ...
        named, label, area, dedupeKey: tag === 'IMG' ? (attrs.src || '') : '',
      });
    }
  }
  return regions;
