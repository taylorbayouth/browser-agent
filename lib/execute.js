...
const { getCachedVisualImage } = require('./visual_enrichment');
  ...
  async function executeAction(action, session, brief) {
    ...
    try {
      ...
      if (action.verb === 'save_image') {
        // Loop-level promotion op. Visual enrichment already captured and
        // classified @v pixels in memory; for other refs (or no ref), capture
        // directly without a vision call. This is the model's explicit decision
        // to persist a visual into assets/ and the manifest.
        ...
        const cached = action.ref ? getCachedVisualImage(brief, action.ref) : null;
        const sourceUrl = cached?.sourceUrl || hiddenSourceUrl(node);
        let captured = null;
        if (sourceUrl) {
          try {
        ...
        captured = captured || (cached?.image ? cached : await captureImage({ session, brief, ref: action.ref }));
        const detail = {
          image: captured.image,
          mimeType: captured.mimeType,
          ext: captured.ext || 'png',
          description: cached?.description || node?.description || node?.name || action.args?.hint || '',
