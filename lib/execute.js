  ...
  async function executeAction(action, session, brief) {
    ...
    try {
      ...
      if (action.verb === 'save_image') {
        ...
        // classified @v pixels in memory; this action is the model's explicit
        // decision to persist that visual into assets/ and the manifest.
        const cached = getCachedVisualImage(brief, action.ref);
        if (!cached?.image) throw new Error(`no cached visual image for ${action.ref}`);
        const detail = {
          image: cached.image,
