...
function computeBriefHash(brief) {
  ...
  const stable = {
    ...
    visuals: brief.visuals?.map(v => ({ role: v.role, description: v.description })) ?? [],
  };
...
// Vision-enriched visual line: the pixels were already inspected before the
// planner sees the page, so the listing carries the compact description instead
// of asking the model to take a screenshot just to discover what's there.
function formatVisual(v, view) {
  const ref = v.ref || '@v?';
  const role = (v.role || 'visual').padEnd(10);
  const b = bboxFromAny(v.bbox);
  const dims = b ? `${Math.round(b.width)}×${Math.round(b.height)} ` : '';
  const desc = truncate(cleanName(v.description), view.maxTextChars).replace(/"/g, '\\"');
  return `[${ref}]  ${role}  ${dims}— "${desc}"${foldSuffix(v)}`;
}
...
function reduce(brief, viewCfg = {}) {
  ...
  // Merge elements, text, visuals, and unreadable regions into one reading-order
  // list so labels sit next to fields and graphics appear where they are on the
  // page.
  const merged = [
    ...elements.map(node => ({ kind: 'e', node, key: sortKey(node.bbox) })),
    ...text.map(node => ({ kind: 't', node, key: sortKey(node.bbox) })),
    ...visuals.map(node => ({ kind: 'v', node, key: sortKey(node.bbox) })),
  ...
  for (const { kind, node } of merged) {
    if (kind === 't') {
      ...
      lastText = null;
      if (cleanName(node.description)) lines.push(formatVisual(node, view));
    } else {
      lastText = null;
