...
// Its @v ref can be inspected with take_screenshot or promoted with save_image.
// A NAMED visual (logo/photo) is saved to SHOW it in the report; an unnamed one
// is inspected to READ it. The optional label echoes its name/title.
function formatRegion(r, view) {
  ...
  let note;
  if (desc) {
    const save = r.referenceImage ? `; save_image ${ref} to save image` : '';
    note = `vision: "${truncate(desc, view.maxTextChars).replace(/"/g, '\\"')}"${save}`;
  } else {
    note = r.named ? `save_image ${ref} to capture` : `unreadable; take_screenshot ${ref} to read`;
  }
