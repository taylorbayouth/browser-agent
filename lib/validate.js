...
function normalizeRef(ref) {
  return typeof ref === 'string' && /^[etv]\d+$/.test(ref) ? '@' + ref : ref;
}
...
function validate(actions, lookup, registry) {
  ...
  for (const action of actions) {
    ...
    // the whole viewport when not). Required refs include save_image. In both
    // cases, a ref that IS present must be
    ...
    if (spec.requiresRef || (spec.optionalRef && refPresent)) {
      ...
      if (!ref) {
        ...
        if (!REF_RE.test(ref)) { errors.push({ action, error: `ref "${ref}" does not match /^@[etv]\\d+$/` }); continue; }
        const refType = ref[1];
    ...
    const argSchema = verb === 'done' ? (spec.args || {}) : { intent: 'string', ...(spec.args || {}) };
    const argErr = checkArgs(action.args, argSchema);
    if (argErr) { errors.push({ action, error: argErr }); continue; }
    if (verb !== 'done') {
      if (!action.args.intent.trim()) {
        errors.push({ action, error: 'arg "intent" must not be empty' });
        continue;
      }
      if (wordCount(action.args.intent) > MAX_INTENT_WORDS) {
        errors.push({ action, error: `arg "intent" must be ${MAX_INTENT_WORDS} words or fewer` });
        continue;
      }
    }
