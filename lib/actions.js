...
const ACTIONS = {
  ...
  take_screenshot: {
    ...
    changesPage: false,
    idempotentRead: true,
    description: 'Capture a visual as report evidence; vision then describes it. Crop-first: pass an exact visible @v/@e/@t ref — never punctuation, selectors, words, or coordinates; no ref grabs the whole viewport (rare fallback). Shoot anything worth showing or reading: charts, diagrams, canvas, scans, CAPTCHA, logos, photos, product shots, key imagery. intent is the saved caption — what it shows and why it matters.',
  },
  get_files: {
    requiresRef: false,
    args: {},
  ...
  save_record: {
    ...
    args: { content: 'string', summary: 'string' },
    changesPage: false,
    description: 'Save one complete final deliverable record. Use only when the record contract required fields are present. content = full record; summary = short record label shown later. Each save_record increments record progress.',
  },
  save_file: {
    requiresRef: false,
    args: { url: 'string' },
