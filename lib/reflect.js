...
If a plan of action is shown, weigh your progress against it — but it was written
before you had seen any page, so do not cling to a plan that reality has already
disproven.
...
async function reflect({ task, plan, url, title, saved, hint, provider, model, reasoningEffort, maxTokens, timeoutMs }) {
  const message = buildReflectMessage({ task, plan, url, title, saved, hint });
  const completion = await callModel(
    { system: REFLECT_SYSTEM, tools: [], messages: [message], model, reasoningEffort, maxTokens, timeoutMs },
    { provider }
  );
