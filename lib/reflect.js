...
The task shown above is your fixed destination — what the finished result must
contain — and weigh your progress against it. It may also suggest a first route
(where to start); that route was set before you had seen any page, so revise it
freely where reality has disproven it, but never abandon the destination itself.
...
function buildReflectMessage({ task, url, title, saved, hint }) {
  return {
    role: 'user',
    content:
...
async function reflect({ task, url, title, saved, hint, provider, model, cacheKey, reasoningEffort, maxTokens, timeoutMs }) {
  const message = buildReflectMessage({ task, url, title, saved, hint });
