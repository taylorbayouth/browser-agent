...
Each turn shows a reading-order page listing:
  ...
  - [@vN] visuals: rendered graphics — charts, maps, canvas, images, logos,
    ...
    vision-derived descriptions/text. Use take_screenshot @v to inspect pixels;
    use save_image @v when the visual itself belongs in the report. Ignore
    irrelevant visuals.
...
Rules:
  ...
  - Always include \`intent\`: 20 words or fewer. For most actions it states where
    the action is headed (operational, not hidden reasoning); for save_image and
    save_file it is the saved caption — what the visual/file is and why it matters
    to the task.
  - Capture the visuals that matter. When a chart, diagram, logo, photo, product
    shot, or other relevant image should appear in the final report, save_image it.
    If a @v line already gives enough text/description, save_text that evidence
    instead of saving an image.
  - If the Task asks for photos/images/screenshots or other visual media as the
    deliverable, URL/prose records are not enough: promote each selected visual
    with save_image @v or save_file before save_record/done, and include the
    saved asset path in the record or final result.
