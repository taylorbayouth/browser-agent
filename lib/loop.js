...
function buildRecordProgressBlock(contract, savedRecordCount) {
  ...
  return [
    ...
    `Fields to capture: ${fieldList(contract.fields)}.`,
    remaining > 0
      ? `Remaining: ${remaining}. Save one ${contract.recordName} record per item with whatever fields you found; a missing field is fine — do not loop hunting for it.`
