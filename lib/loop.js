...
const { callModel, toolsFromRegistry, providers } = require('./plan');
...
async function run({ session, task, config = {} } = {}) {
  ...
  const visionCfg = cfg.models.vision || {};
  ...
  if ((cfg.models.reflect || {}).enabled !== false) {
    const rp = cfg.models.reflect?.provider || provider;
    const rm = cfg.models.reflect?.model || plannerModel;
    modelsLine += ` · reflect ${rp}:${rm}`;
  }
  if ((cfg.models.plan || {}).enabled !== false) {
    const pp = cfg.models.plan?.provider || provider;
    const pm = cfg.models.plan?.model || plannerModel;
    modelsLine += ` · plan ${pp}:${pm}`;
  }
  emit(modelsLine);
  ...
  try {
    ...
    const planCfg = cfg.models.plan || {};
    if (planCfg.enabled !== false) {
      try {
        const { text, completion } = await generatePlan({
          task,
          context: cfg.context,
          provider: planCfg.provider || provider,
          model: planCfg.model || model,
...
async function finish(runArtifact, { scratchpad, config = {} } = {}) {
  if (scratchpad) {
    ...
    if (reportCfg.enabled === false) {
      ...
      try {
        const reportProvider = reportCfg.provider || config.models?.primary?.provider;
        const reportModel = reportCfg.model || config.models?.primary?.model;
