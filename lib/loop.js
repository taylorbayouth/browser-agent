...
const { reflect, clipSaved, REFLECT_SYSTEM, buildReflectMessage } = require('./reflect');
const { generatePlan, buildPlanSystem, buildPlanMessage } = require('./planning');
const { generateReport, fallbackReport, selectReportEvidence, REPORT_SYSTEM, buildReportMessage } = require('./report');
const { estimateTokens } = require('./tokens');
const { cleanUrl } = require('./url');
const { createVisualEvidence } = require('./visual-evidence');

const sleep = (ms) => new Promise(r => setTimeout(r, ms));

const VRULE = '═'.repeat(60);
function emitPrompts(emit, label, { system, user } = {}) {
  emit(`\n${VRULE}`);
  emit(`[verbose] ${label}`);
  if (system != null) {
    emit(`[verbose] SYSTEM:\n${system}`);
...
// Strip the DOM refs listing from a turn message so verbose output stays readable.
// The refs block runs from "Refs (valid only now):\n" up to "Choose one best next action."
function stripRefs(content) {
  return content.replace(
    /\nRefs \(valid only now\):[\s\S]*?(?=\nChoose one best next action\.)/,
    '\n[DOM refs omitted — run without --verbose to see full listing]'
  );
}
...
function describeAction(action, brief) {
  ...
  switch (action.verb) {
    ...
    case 'take_screenshot': line = 'inspected a screenshot'; break;
    case 'save_image': line = 'saved image'; break;
...
function buildRecordProgressBlock(contract, savedRecordCount) {
  if (!contract) return null;
  const remaining = Math.max(0, contract.target - savedRecordCount);
  const reqKeys = Object.keys(contract.requiredFields || {});
  const optKeys = Object.keys(contract.optionalFields || {});
  const fieldsLine = [
    reqKeys.length ? `Required: ${reqKeys.join(', ')}` : null,
    optKeys.length ? `Optional (skip if not visible): ${optKeys.join(', ')}` : null,
  ].filter(Boolean).join(' | ') || '(none)';
  return [
    `Record target: ${contract.target} ${contract.recordName} record${contract.target === 1 ? '' : 's'}.`,
    `Saved records: ${savedRecordCount}/${contract.target}.`,
    `Fields — ${fieldsLine}.`,
    remaining > 0
      ? `Remaining: ${remaining}. Save one ${contract.recordName} record per item with all required fields; optional fields only if visible.`
      : 'Record target reached — stop collecting records. Finish any remaining requirements, then call done.',
  ].join('\n');
}
...
function buildTurnMessage(events, llmView, revisits = 0, pivot = null, progressBlock = null) {
...
${RULE}\n`
    : '';
  // The task is NOT restated here — it lives in the cached system prompt (see
  // lib/prompt.js buildTaskSection). This message carries only what changes each
  // turn: history, record progress, any reflection pivot, and the current page.
  return {
    role: 'user',
    content:
`History:
...
async function extractBrief(session, allowFreshTab = true, visualEvidence = null) {
  ...
  if (visualEvidence) await visualEvidence.enrich({ session, brief });
...
async function nextChangedBrief(session, lastHash, loopCfg, visualEvidence = null) {
  ...
  let brief = await extractBrief(session, true, visualEvidence);
  ...
  while (brief.briefHash === lastHash) {
    ...
    brief = await extractBrief(session, false, visualEvidence);
  }
...
// The http(s) href of a link the model just clicked, or null if the action
// wasn't a click on a real link. Lets the loop tell, next turn, whether that
// click navigated — a link that didn't move us off the page is dead.
function clickedLinkHref(brief, action) {
  if (!action || action.verb !== 'click' || !action.ref) return null;
  const node = (brief.elements || []).find(n => n.ref === action.ref)
    ?? (brief.text || []).find(n => n.ref === action.ref);
  const url = node?.url;
...
function idempotentReadKey(brief, action) {
  ...
  if (action.verb === 'take_screenshot' || action.verb === 'save_image') {
    return action.ref ? `${action.verb}|${targetKey(brief, action.ref)}` : `${action.verb}|viewport`;
  }
  return action.verb;
}

// Pure no-op / flailing detector. Decides whether THIS turn's planned action
// repeats last turn's with no real progress, and which of four shapes tripped
// (first match wins; `shape` is for the stuck log event, not control flow).
// Behavior is identical to the inline predicates it replaces — extracted so each
// shape is table-testable in isolation. The shapes differ in how they gate on
// `changed` precisely because different no-ops leave the page in different states:
//  - sameAction: the exact same action again after the page DIDN'T change — the
//    original dead click/press, confirmed by the change-poll.
//  - repeatedRead: an idempotentRead (screenshot/list) repeated on a page whose
//    hash matches the one it last ran on. Reads null lastHash, so `changed` is
//    always true and sameAction can't see them — keyed on the brief hash instead.
//  - sameErroredTarget: re-planning the exact action that ERRORED last turn.
//    Independent of `changed` — an errored action leaves the page untouched yet
//    nulls lastHash, making `changed` misleadingly true.
//  - sameTypeRepeat: the exact same `type` again, even when `changed` is true. A
//    silent clear-no-op appends instead of replacing, growing the field and
//    flipping the hash — so it counts regardless of `changed`. Gated to `type`
//    because a repeated click on "Load more" IS legitimate progress.
function stuckSignal({
  changed, primaryKey, primaryReadKey, primaryVerb,
  lastActionKey, lastReadKey, lastActionBriefHash, lastErroredKey, briefHash,
} = {}) {
  ...
  const repeatedRead = !!primaryReadKey && primaryReadKey === lastReadKey
    && lastActionBriefHash != null && briefHash === lastActionBriefHash;
  const sameErroredTarget = !!lastErroredKey && !!primaryKey && primaryKey === lastErroredKey;
  const sameTypeRepeat = primaryVerb === 'type' && !!lastActionKey && primaryKey === lastActionKey;
  let shape = null;
  if (sameAction) shape = 'sameAction';
  else if (repeatedRead) shape = 'repeatedRead';
  else if (sameErroredTarget) shape = 'sameErroredTarget';
  else if (sameTypeRepeat) shape = 'sameTypeRepeat';
  return { tripped: shape !== null, shape };
}
...
function termDesc(action, brief, observation) {
  ...
  switch (action.verb) {
    ...
    case 'take_screenshot': return observation?.detail?.cropped ? 'Inspected screenshot (cropped)' : 'Inspected screenshot';
    case 'save_image': {
...
async function run({ session, task, config = {} } = {}) {
  ...
  const visualEvidence = createVisualEvidence(cfg.visualEvidence || {});
  const runMode = ['auto', 'records', 'research'].includes(cfg.mode) ? cfg.mode : 'auto';
  ...
  const runArtifact = {
    ...
    task,
    context: cfg.context ?? null,   // trusted operator background fed to Step 0 + every turn
    model: model || 'default',
    startedAt: new Date().toISOString(),
    endedAt: null,
    status: 'running',
    mode: runMode,
    taskType: null,             // records | research, from Step 0; null if disabled/failed
    expandedTask: null,         // Step 0's rewrite of `task` (lib/planning.js); null if disabled/failed
    requirements: [],           // Step 0's enumerated requirements checklist; [] if disabled/failed
    recordContract: null,       // Step 0's final-record ledger contract; null if disabled/failed
    records: [],
    result: null,
    reflections: [],            // mid-run pivots: {turn, reason, decision} (see maybeReflect)
    steps: [],
    completions: [],
    stats: {
      stepCount: 0,
  ...
  let modelsLine = `models · primary ${roleLabel('primary')} · vision ${roleLabel('vision', { vision: true })}`;
  if ((cfg.models.reflect || {}).enabled !== false) modelsLine += ` · reflect ${roleLabel('reflect')}`;
  if ((cfg.models.expand || {}).enabled !== false) modelsLine += ` · expand ${roleLabel('expand')}`;
  ...
  // Links whose hit-point doesn't navigate (e.g. a SERP result whose clickable
  // box is the wrong sub-region). Keyed by href — refs are regenerated every
  // snapshot, but the destination URL is stable. reduce demotes these to
  // non-clickable lines so the model keeps the URL (and can navigate to it) but
  // can't re-click the dead target and spin.
  const deadLinks = new Set();
  // The href of a link clicked last turn + the page it was clicked on, so the
  // next turn can tell whether that click actually navigated (see deadLinks).
  ...
  // Cooldown clock between reflections. Measured in totalIterations (a monotonic
  // pass counter), NOT iter: a reflection refunds iter (iter--), so on the next
  // pass iter climbs back to its pre-reflection value and an iter-based cooldown
  // would read distance 0 — re-extending the cooldown by every reflection and
  // suppressing a reflection on a genuinely NEW stall a couple of turns later.
  let lastReflectTurn = -Infinity;
  ...
  async function maybeReflect(reason, brief, hint = null) {
    ...
    if (cfg.verbose) {
      const reflectMsg = buildReflectMessage({
        task: runArtifact.expandedTask || task, url: brief?.url, title: brief?.title, saved, hint,
      });
      emitPrompts(emit, `REFLECT (reason: ${reason}) — sees the expanded task`, {
        system: REFLECT_SYSTEM,
        user: reflectMsg.content,
      });
    }
    let decision = '';
    try {
    ...
    events.push(line);
    emit(`\n${line}`);
    runArtifact.reflections.push({ turn: iter, reason, decision });
  ...
  // `onReflected` runs the instant a reflection fires, before returning — this is
  // where a guard resets its own counter (visitCounts, scroll reversals, the
  // budget latch). Putting it here, rather than after the call, makes the
  // reset-before-pivot contract impossible to get wrong: a guard can never latch
  // state on a reflection that didn't actually fire (the original #9 budget bug).
  async function escalate(reason, brief, { hint = null, onUnavailable = 'warn', onReflected, status, result, emitLine, logEvent } = {}) {
    if (await maybeReflect(reason, brief, hint)) {
      if (onReflected) onReflected();
      return 'reflected';
    }
  ...
  const onSigint = () => {
    ...
    try {
      const evidence = selectReportEvidence({
        ...
        rawTokenBudget: cfg.models.report?.rawTokenBudget,
      });
  ...
  try {
    ...
    // ─── Step 0: task expansion ───────────────────────────────────────────────
    // One upfront call (lib/planning.js) that rewrites the bare task into one
    // complete instruction, formed from the task + trusted context before any page
    // is seen. It's an immutable north star: appended to the planner's system
    // prompt below (cached, every turn), and handed to each reflection
    // (maybeReflect) and the final report (finish) so every stage shares one
    // reading of the task. A failure here must never break the run — proceed with
    // the raw operator task verbatim, as before.
    const expandCfg = resolveRole(cfg, 'expand');
    if (expandCfg.enabled !== false) {
      if (cfg.verbose) {
        emitPrompts(emit, 'STEP 0 — PLANNING (produces the expanded task)', {
          system: buildPlanSystem(runMode),
          user: buildPlanMessage({ task, context: cfg.context }).content,
        });
      }
      try {
        const { task: expandedTask, taskType, recordContract, requirements, completion } = await generatePlan({
          task,
          context: cfg.context,
          mode: runMode,
          provider: expandCfg.provider,
          model: expandCfg.model,
          cacheKey: 'browser-agent:plan',
          reasoningEffort: expandCfg.reasoningEffort,
          maxTokens: expandCfg.maxTokens,
          timeoutMs: expandCfg.timeoutMs,
        });
        runArtifact.completions.push(completion);
        runArtifact.stats.totalInputTokens += completion.usage?.inputTokens || 0;
        runArtifact.stats.totalOutputTokens += completion.usage?.outputTokens || 0;
        if (!runArtifact.model || runArtifact.model === 'default') runArtifact.model = completion.model;
        if (taskType) {
          runArtifact.taskType = taskType;
          logger.event({ kind: 'task-type', taskType, mode: runArtifact.mode });
        }
        if (expandedTask) {
          runArtifact.expandedTask = expandedTask;
          logger.event({ kind: 'expanded-task', task: expandedTask });
          emit(`\ntask · ${expandedTask}`);
        }
        if (Array.isArray(requirements) && requirements.length) {
          runArtifact.requirements = requirements;
          logger.event({ kind: 'requirements', requirements });
          emit(`requirements · ${requirements.length}\n${requirements.map((r, i) => `  ${i + 1}. ${r}`).join('\n')}`);
        }
        if (taskType === 'records' && recordContract) {
          runArtifact.recordContract = recordContract;
          logger.event({ kind: 'record-contract', recordContract });
          emit(`\nrecords · ${recordContract.target} ${recordContract.recordName}${recordContract.target === 1 ? '' : 's'}`);
        } else if (taskType === 'research') {
          emit('\nmode · research');
        }
      } catch (err) {
        // Step 0 is an enhancement, never a dependency (see comment above).
        emit(`  (task expansion skipped: ${err?.message || err})`);
      }
    }

    // The task the agent actually runs against: Step 0's rewrite, or the raw
    // operator task verbatim if Step 0 was disabled or failed.
    const effectiveTask = runArtifact.expandedTask || task;

    // Build the planner's prompt now, AFTER Step 0, so the immutable task rides
    // along inside the cached system prefix on every turn (see lib/prompt.js).
    const system = buildSystemPrompt(actions, cfg.context, effectiveTask, runArtifact.requirements);
    if (cfg.verbose) {
      emitPrompts(emit, 'LOOP SYSTEM PROMPT (sent every turn; contains the expanded task)', { system });
    }
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      let { brief, changed } = await nextChangedBrief(session, lastHash, loopCfg, visualEvidence);
      ...
      // Dead-link detection: if last turn we clicked a link and we're still on
      // the same page, that link's hit-point doesn't navigate. Record its href
      // so reduce demotes it to a non-clickable line (URL kept, ref dropped) —
      // the model can navigate to the URL but can't re-click the dead target and
      // loop. Keyed on the URL we're still on (not the flaky brief hash, which a
      // panel toggle can flip without any navigation).
      if (lastClickedLink && brief.url === lastClickedLink.fromUrl) {
        deadLinks.add(lastClickedLink.href);
      }
      ...
      if (arrived && maxVisits && visits >= maxVisits) {
        ...
        // onReflected resets this URL's visit count so the pivot isn't re-fired next turn.
        if (await escalate('url-revisit', brief, { hint, onReflected: () => visitCounts.set(curUrl, 0) }) === 'reflected') {
      ...
      if (!budgetReflected && iter >= budgetTurn) {
        // onReflected latches the one-shot budget flag only if the reflection fired.
        if (await escalate('budget', brief, { onReflected: () => { budgetReflected = true; } }) === 'reflected') {
          iter--;   // the reflection turn doesn't consume the step budget
          continue;
        }
      }
      ...
      // 2. Reduce
      const llmView = reduce(brief, cfg.view, deadLinks);
      ...
      const turnMessage = buildTurnMessage(
        ...
        revisits,
        pendingPivot,
        buildRecordProgressBlock(runArtifact.recordContract, savedRecordCount)
      );
      pendingPivot = null;  // shown once; the next reflection sets it again
      if (cfg.verbose) {
        emitPrompts(emit, `LOOP TURN ${iter} — USER MESSAGE (history + page; system prompt unchanged)`, {
          user: stripRefs(turnMessage.content),
        });
      }
      ...
      const stuck = stuckSignal({
        changed,
        primaryKey,
        primaryReadKey,
        primaryVerb: primary?.verb ?? null,
        lastActionKey,
        lastReadKey,
        lastActionBriefHash,
        lastErroredKey,
      ...
      stuckStreak = stuck.tripped ? stuckStreak + 1 : 0;
      if (stuckStreak >= loopCfg.maxStuckRepeats) {
        ...
        const e = await escalate('stuck', brief, {
          ...
          logEvent: { kind: 'stuck', turn: iter, action: primary, repeats: stuckStreak, shape: stuck.shape },
        });
        if (e === 'reflected') { iter--; continue; }
        if (e === 'aborted') return await finish(runArtifact, { scratchpad, config: cfg });
      }
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        if (obs.verb === 'save_record' && obs.detail) {
          const preview = compactPreview(obs.detail.content);
          savedRecordCount++;
          obs.detail.recordNumber = savedRecordCount;
          obs.detail.recordTarget = runArtifact.recordContract?.target || null;
          const record = {
            number: savedRecordCount,
            summary: obs.detail.summary,
            content: obs.detail.content,
            url: brief.url,
          };
          runArtifact.records.push(record);
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        if (failed) {
          ...
          // save_text/save_record: the full content is on disk; a bounded
          // preview re-enters context so saved facts remain usable after
          // navigation.
          note = ` — "${step.observation.detail.summary}"`;
          if (step.observation.detail.preview) note += ` — saved: "${step.observation.detail.preview}"`;
          if (step.observation.detail.recordNumber) {
            const n = step.observation.detail.recordNumber;
            const target = step.observation.detail.recordTarget;
            note += target ? ` — record ${n}/${target}` : ` — record ${n}`;
          }
          const saved = step.observation.detail.savedPath;
          if (saved) note += ` → ${saved}`;
        }
      ...
      // Records have no auto-stop. The contract counts the deliverable — surfaced in
      // the per-turn progress block and the `record N/target` notes — but reaching the
      // target never ends the run on its own, because a records task often carries
      // follow-on requirements (message each contact, compare, rank). The model
      // finishes by calling `done` once every requirement is satisfied (see the
      // Requirements checklist in lib/prompt.js); the maxSteps ceiling is the backstop.
      ...
      // If we clicked a link, remember its href + the page we clicked it on, so
      // next turn we can tell whether it navigated (see the dead-link check).
      const clickedHref = primaryExecuted ? clickedLinkHref(brief, primaryExecuted) : null;
      lastClickedLink = clickedHref ? { href: clickedHref, fromUrl: brief.url } : null;
      ...
      if (scrollEscalate) {
        ...
        // onReflected zeroes the reversal count so the pivot isn't re-fired next turn.
        if (await escalate('scroll-oscillation', brief, { hint, onReflected: () => { scrollLoopState.reversals = 0; } }) === 'reflected') {
          iter--;   // the reflection turn doesn't consume the step budget
          continue;
        }
      }
  ...
  if (scratchpad) {
    ...
    if (reportCfg.enabled === false) {
      ...
      try {
        ...
        if (config.verbose) {
          const reportMsg = buildReportMessage({
            task: runArtifact.expandedTask || runArtifact.task,
          ...
          process.stderr.write(`\n${VRULE}\n[verbose] REPORT — sees the expanded task\n[verbose] SYSTEM:\n${REPORT_SYSTEM}\n[verbose] USER:\n${reportMsg.content}\n${VRULE}\n`);
        }
...
module.exports = { run, scrollPatternSignal, stuckSignal };
