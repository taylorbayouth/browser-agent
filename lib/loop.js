...
function buildRecordProgressBlock(contract, savedRecordCount) {
  ...
  return [
    ...
    `Required fields: ${fieldList(contract.requiredFields)}.`,
    `Optional fields: ${fieldList(contract.optionalFields)}.`,
    remaining > 0
      ? `Remaining: ${remaining}. Use save_record only for one complete ${contract.recordName} record with all required fields.`
...
function buildTurnMessage(task, events, llmView, revisits = 0, pivot = null, progressBlock = null) {
...
async function run({ session, task, config = {} } = {}) {
  ...
  try {
    ...
    if (planCfg.enabled !== false) {
      try {
        const { text, taskType, recordContract, completion } = await generatePlan({
          task,
          context: cfg.context,
          mode: runMode,
