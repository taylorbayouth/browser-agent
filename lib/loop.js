    ...
    while (iter < loopCfg.maxSteps) {
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        if (failed) {
          ...
          if (step.observation.detail.completeItemNumber) {
            const n = step.observation.detail.completeItemNumber;
            const target = step.observation.detail.completeItemTarget;
            note += target ? ` — complete item ${n}/${target}` : ` — complete item ${n}`;
          }
          const saved = step.observation.detail.savedPath;
          if (saved) note += ` → ${saved}`;
        }
      ...
      if (itemQuota && completeItemCount >= itemQuota.count) {
        runArtifact.status = 'completed';
        runArtifact.result = `Collected requested ${itemQuota.count} ${itemQuota.label}.`;
        emit(`  ✓ target reached — ${completeItemCount}/${itemQuota.count} ${itemQuota.label}`);
        logger.event({
          kind: 'quota-complete',
          turn: iter,
          target: itemQuota.count,
          label: itemQuota.label,
          completeItemCount,
        });
        return await finish(runArtifact, { scratchpad, config: cfg });
      }
