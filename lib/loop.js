...
const { enrichVisualEvidence } = require('./visual_enrichment');
...
function briefNodeCount(brief) {
  return (brief.elements?.length || 0) + (brief.text?.length || 0) + (brief.regions?.length || 0) + (brief.visuals?.length || 0);
}
...
async function retrySparseNavigationBrief(session, brief, prevUrl, loopCfg, cfg = DEFAULTS) {
  ...
  for (let i = 0; i < maxRetries && isSparseBrief(current, loopCfg); i++) {
    await sleep(retryMs);
    current = await extractBrief(session, false, cfg);
  }
  return current;
}
...
async function extractBrief(session, allowFreshTab = true, cfg = DEFAULTS) {
  ...
  const rawHash = brief.briefHash ?? computeBriefHash(brief);
  await enrichVisualEvidence({ session, brief, config: cfg.visualEvidence, rawHash });
  brief.briefHash = computeBriefHash(brief);
  return brief;
}
...
async function nextChangedBrief(session, lastHash, loopCfg, cfg = DEFAULTS) {
  ...
  let brief = await extractBrief(session, true, cfg);
  ...
  while (brief.briefHash === lastHash) {
    ...
    brief = await extractBrief(session, false, cfg);
  }
...
async function run({ session, task, config = {} } = {}) {
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      let { brief, changed } = await nextChangedBrief(session, lastHash, loopCfg, cfg);
      brief = await retrySparseNavigationBrief(session, brief, prevUrl, loopCfg, cfg);
      ...
      // Persist only explicit image-promotion actions. Read-only screenshots
      // still return pixels for vision, but their base64 is stripped before logs
      // and never lands in assets/ unless the model calls save_image.
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        if (failed) {
          ...
          const saved = step.observation.detail.savedPath;
          if (saved) note += ` → ${saved}`;
        } else if (step.action.verb === 'save_record' && step.observation.detail?.savedPath) {
          note = ` → ${step.observation.detail.savedPath}`;
        }
