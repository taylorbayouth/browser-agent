...
function buildRecordProgressBlock(contract, savedRecordCount) {
  ...
  return [
    ...
    remaining > 0
      ...
      : 'Target reached. Call done now; do not search for extras.',
  ].join('\n');
}
...
async function run({ session, task, config = {} } = {}) {
  ...
  try {
    ...
    if (expandCfg.enabled !== false) {
      try {
        const { task: expandedTask, taskType, recordContract, completion } = await generatePlan({
          task,
          context: cfg.context,
          mode: runMode,
    ...
    const system = buildSystemPrompt(actions, cfg.context, effectiveTask);
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      const recordContract = runArtifact.recordContract;
      if (recordContract && savedRecordCount >= recordContract.target) {
        runArtifact.status = 'completed';
        const plural = recordContract.target === 1 ? recordContract.recordName : `${recordContract.recordName} records`;
        runArtifact.result = `Collected requested ${recordContract.target} ${plural}.`;
        emit(`  ✓ record target reached — ${savedRecordCount}/${recordContract.target} ${recordContract.recordName}`);
        logger.event({
          kind: 'record-target-complete',
          turn: iter,
          target: recordContract.target,
          recordName: recordContract.recordName,
          savedRecordCount,
        });
        return await finish(runArtifact, { scratchpad, config: cfg });
      }
