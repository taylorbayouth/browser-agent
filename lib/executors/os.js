...
const DEFAULT_HUMANIZE = {
  ...
  scrollJitterPx: 3,
  cursorToOmnibox: true,
};

// How far above the viewport top (CSS px) the cosmetic navigate-glide aims, to
// land the cursor roughly over Chrome's address bar. The omnibox sits inside the
// chrome strip whose exact height varies (bookmarks bar, zoom); this is a
// look-right target, never clicked, so an approximate inset is all it needs.
...
//
// What we DO add (cursorToOmnibox) is purely cosmetic: glide the real cursor up
// toward the address bar so a navigate reads as a deliberate "type a URL" beat in
// a demo, instead of the page silently swapping under a parked cursor. It only
// MOVES — never clicks, never types — so none of the Cmd+L failure modes apply,
// and it cannot affect the next action's aim: every click recomputes an absolute
// screen target (clickablePoint → pageToScreen → exact-target snap), so the
// cursor's resting spot is irrelevant.

// Park the cursor over Chrome's address bar. Best-effort and self-contained: a
// decorative move must never break (or fail) a navigation, so every error is
// swallowed. The omnibox lives in browser chrome above the page, which CDP can't
// measure, so we map the top-center of the viewport to screen coords and step up
// OMNIBOX_INSET_PX into the chrome.
async function moveCursorToOmnibox({ session, client, humanize }) {
  try {
    await ensureInputSafe(client);
    ...
    // viewportRelative: pageY 0 is the viewport top; the omnibox is just above it.
    ...
    if (process.env.BROWSER_AGENT_DEBUG_NAV) console.error(`[nav] cursor-to-omnibox skipped: ${err?.message || err}`);
  }
}

// OS-backend navigate: kick off the cosmetic glide and the real navigation at the
// same time. navigate() blocks on the page's load + network-idle (lib/executors/
// page.js), and the glide animates inside that window we already pay for — so the
// visible motion costs ~0 extra latency. Fire-and-forget (not awaited): the native
// helper serializes its op queue, so by the time the next action's move is sent the
// glide has long drained; meanwhile the loop's next extract (screenshot + LLM) more
// than covers the few hundred ms it takes. A still-in-flight glide during the next
// screenshot is, if anything, a more lifelike demo frame.
async function osNavigate({ session, url, client, humanize }) {
  if (humanize.enabled && humanize.cursorToOmnibox !== false) {
    moveCursorToOmnibox({ session, client, humanize }).catch(() => {});
  }
...
function create(opts = {}) {
  ...
  return {
    ...
    navigate: (args) => osNavigate({ ...args, client, humanize }),   // CDP Page.navigate + cosmetic cursor glide
