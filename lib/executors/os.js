...
const { screenshot: inspectScreenshot, saveImage } = require('../screenshot');
...
function create(opts = {}) {
  ...
  const startupDelayMs = Number.isFinite(opts.startupDelayMs) ? opts.startupDelayMs : 4000;
  ...
  return {
    ...
    async waitUntilReady() {
      if (!startupDone) {
        if (startupDelayMs > 0) {
          console.error(`[os] startup delay: waiting ${startupDelayMs}ms before sending input.`);
    ...
    take_screenshot: (args) => inspectScreenshot(args),
