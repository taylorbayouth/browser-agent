...
function filenameStem(text, maxLength = 80) {
...
function screenshotTitle({ title, reason, count } = {}) {
  const captioned = cleanWebText(reason || '').replace(/\s+/g, ' ').trim().slice(0, 80);
  if (captioned) return /\bscreenshot$/i.test(captioned) ? captioned : `${captioned} screenshot`;
...
function createScratchpad({ enabled = true, dir = path.resolve(process.cwd(), 'runs'), runId } = {}) {
  if (!enabled) {
    return {
      saveText() { return null; }, saveRecord() { return null; }, saveImage() { return null; },
  ...
  function safeWrite(fn) {
    try { fn(); } catch (err) {
      if (!warnedWrite) {
        warnedWrite = true;
        process.stderr.write(`[scratchpad] write failed (further errors suppressed): ${err.message}\n`);
      }
    }
  }
  ...
  function appendIndex({ type, title, url, source, reason, assetLabel, assetPath, summary } = {}) {
    ...
    safeWrite(() => fs.appendFileSync(indexPath, block + '\n'));
  }
  ...
  function saveText({ content, summary, title, url, reason } = {}) {
    ...
    safeWrite(() => fs.appendFileSync(savedPath, block + '\n'));
    appendIndex({
      type: 'text',
  ...
  function saveRecord({ content, summary, url, reason } = {}) {
    if (content == null) return null;
    saveCount++;
    recordCount++;
    const recordId = `save-${saveCount}`;
    const text = cleanWebText(String(content));
    const block = [
      summary ? `### Record ${recordCount}: ${summary}` : `### Record ${recordCount}`,
      url ? `- URL: ${url}` : null,
      reason ? `- Reason: ${String(reason).trim()}` : null,
      '',
      text,
    ...
    appendIndex({
      type: 'record',
      title: summary || `Record ${recordCount}`,
      url,
      reason,
      summary: summary || text,
    });
    return { path: savedPath, name: 'saved.md', count: recordCount, recordId };
  }
  ...
  function saveImage({ base64, title, url, description, id, ext = 'png', reason } = {}) {
    if (!base64) return null;
    fs.mkdirSync(assetsDir, { recursive: true });
    saveCount++;
    imageCount++;
    const recordId = `save-${saveCount}`;
    const reasonStem = filenameStem(reason);
    const stem = reasonStem ? `${reasonStem}-screenshot-${id || imageCount}` : `screenshot-${imageCount}`;
    const name = uniqueAssetName(assetsDir, `${stem}.${ext}`, imageCount);
    const imgPath = path.join(assetsDir, name);
    safeWrite(() => fs.writeFileSync(imgPath, Buffer.from(base64, 'base64')));
    const displayTitle = screenshotTitle({ title, reason, count: imageCount });
    const block = [
      `### ${displayTitle}`,
      url ? `- URL: ${url}` : null,
      reason ? `- Reason: ${String(reason).trim()}` : null,
  ...
  function saveAsset({ base64, filename, summary, url, id, reason } = {}) {
    if (!base64) return null;
    fs.mkdirSync(assetsDir, { recursive: true });
    saveCount++;
    assetCount++;
    ...
    const stem = filenameStem(reason) || filenameStem(parsed.name) || 'download';
    let name = `${stem}-file-${id || assetCount}${parsed.ext}`;
    name = uniqueAssetName(assetsDir, name, assetCount);
    const filePath = path.join(assetsDir, name);
    safeWrite(() => fs.writeFileSync(filePath, Buffer.from(base64, 'base64')));
    const block = [
  ...
  function writeReport(markdown, { title = 'Report', layout, extraCss, className } = {}) {
    fs.mkdirSync(runDir, { recursive: true });
    safeWrite(() => fs.writeFileSync(reportPath, String(markdown ?? '')));
    safeWrite(() => fs.writeFileSync(reportHtmlPath, markdownToHtmlDocument(markdown, { title, layout, extraCss, className })));
    return reportPath;
  }
...
module.exports = { createScratchpad, filenameStem, compactWords, INDEX_SUMMARY_WORDS };
