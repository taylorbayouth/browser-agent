...
// model got from get_files (or a link it sees in the listing) — and hands them
// back for the loop to persist under assets/. On-page visuals are captured with
// take_screenshot, not here; an image URL passed in still gets a vision note,
// while other files get a metadata note.
//
...
async function saveFile({ session, brief, url, signal } = {}) {
  if (!session?.client) throw new Error('save_file requires a CDP session');
  if (!url || typeof url !== 'string') throw new Error('save_file requires a url (use get_files to find one, or pass a visible link)');
  ...
  if (mime && mime.startsWith('image/') && buf.length > MAX_VISION_BYTES) {
    ...
    const described = vision.normalizeVisionResult(
      await vision.describe({ imageBase64: buf.toString('base64'), mimeType: mime, signal }),
    );
...
module.exports = { saveFile, download, sniff, MAX_DOWNLOAD_BYTES, MAX_VISION_BYTES };
