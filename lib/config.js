...
const DEFAULTS = {
  mode: 'auto',                  // auto | records | research. Controls whether
                                 // Step 0 may create a record ledger target.
  ...
  models: {
    // 0. EXPAND — "Step 0". One call made once at run start (lib/planning.js),
    //    before the agent touches the page, that rewrites the bare task into one
    //    complete, self-contained instruction: every detail preserved, gaps filled,
    //    strengthened for an LLM, and phrased in the agent's verbs. The rewritten
    //    task is an immutable north star, threaded into the planner's system prompt
    //    (every turn), each reflection, and the final report — so every stage shares
    //    one reading of the task. Built from the task + trusted context only (never
    //    page content), so it adds no prompt-injection surface. If disabled or the
    //    call fails, the run simply proceeds with the raw operator task verbatim,
    //    exactly as before. Keep this interactive: it is a classifier/rewrite,
    //    not a deep reflection turn.
    expand: {
      enabled: true,
      provider: 'openai',
      model: null,                 // null → inherit the primary model/provider
    ...
    primary: {
      ...
      maxTokens: 4096,
      // High-effort reasoning + a tool-call response routinely exceeds 10s; that
      // tight deadline made transient stalls fatal. Give it room (above expand's
      // 20s); retries are the backstop for a genuine hang, not this ceiling.
      timeoutMs: 30000,
    },

    // 2. VISION — reads transient visual evidence and describes saved screenshots.
    //    Independent of the planner, so a cheap planner can pair with strong vision.
    vision: {
      provider: 'openai',
  ...
  visualEvidence: {
    enabled: true,                // pre-brief cropped vision pass for @v regions
    maxRegions: 8,                // cap automatic analysis per brief
  },
...
function applyEnvOverrides(cfg) {
  ...
  if (process.env.BROWSER_AGENT_MODE) cfg.mode = process.env.BROWSER_AGENT_MODE;
...
const ROLE_FIELDS = {
  expand:  ['enabled', 'provider', 'model', 'reasoningEffort', 'maxTokens', 'timeoutMs'],
  ...
  report:  ['enabled', 'provider', 'model', 'reasoningEffort', 'maxTokens', 'timeoutMs', 'rawTokenBudget'],
};
...
function validateModels(models) {
  ...
  for (const [role, spec] of Object.entries(models)) {
    ...
    for (const [field, value] of Object.entries(spec)) {
      ...
      } else if (['maxTokens', 'timeoutMs', 'maxReflections', 'cooldownTurns', 'savedMaxChars', 'rawTokenBudget'].includes(field) && value != null && !Number.isFinite(value)) {
