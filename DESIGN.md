...
```jsonc
{
  ...
  "regions":  [ { "ref": "@v1", "role": "canvas", "bbox": [...], "inViewport": true, "named": false, "label": "" }, … ],
  "lookup":   { "@e1": 1276, "@t1": 1419, "@v1": 1502 },
  "stats":    { … }
}
```
...
**Visuals** (`regions`, surfaced as `@v` refs) are rendered graphics the accessibility tree can't describe in text — a `<canvas>`, an `<img>`, an `<svg>`, a URL-backed CSS `background-image`, or a **cross-origin `<iframe>`**. `extract.js` reads them straight from the DOMSnapshot (tag + attributes + layout) and emits one for each rendered graphic that is **not nested in a link/button** (those are control icons, already represented by the control). Two kinds fall out of the accessible-name check:

- **Unnamed** — no `alt`/`aria-label`/`<title>` (a chart, map, scan, CAPTCHA, alt-less image, CSS background image, or cross-origin iframe). The tree is silent, so the pre-brief visual pass may crop and read it before the planner sees the listing.
- **Named** — carries an accessible name (a logo, photo, or product shot with `alt`/`aria-label`). Its content is described by the page, but the pre-brief visual pass can still decide whether it is useful visual evidence. An `<img>` with an explicit empty `alt=""` is decorative and dropped.

This is a deterministic structural fact, not a heuristic about final-report importance: we report that a graphic is painted here, and the planner decides whether to save it. A **named** visual must also clear a size floor (`MIN_VISUAL_AREA`, ~80×80 — below it is an avatar/icon), is deduped by source, and is capped per brief (`NAMED_VISUAL_CAP`, biggest first) so a gallery or avatar list can't flood the listing. **Unnamed** graphics are never floored or capped. CSS background images are surfaced only when they have a URL, are rendered, are non-tiny, and are not repeating.
...
Before the planner sees the brief, `visual-evidence.js` may crop up to `visualEvidence.maxRegions` `@v` regions in memory and ask the vision model for `{description,text,omit,referenceImage}`. `omit:true` removes blank/failed/decorative/layout-only regions from the listing. Useful `text` becomes a synthetic derived `@t` node, so the planner can use `save_text` like it does for normal page text. Useful `description` enriches the existing `@v` line. These crops are transient and stored non-enumerably on the in-memory region object; they are not written to `assets/`, `saved.md`, logs, or report files.

Each remaining visual gets a `@v` ref and a `lookup` entry (like `@e`/`@t`), and renders in the LLMView as an `[@vN]` line in reading order. A plain visual carries the note `save_image @v to capture` (named) or `take_screenshot @v to read` (unnamed); a vision-enriched visual carries the model-derived description and may suggest `save_image @v to save image` when it looks report-worthy. The ref is accepted by `take_screenshot` and `save_image` — `click`/`type`/`selectText` reject `@v` at validation. When either visual action is given a ref (`@e`/`@t`/`@v`), the executor resolves it to that node's bbox and passes a `clip` to `Page.captureScreenshot` with `captureBeyondViewport:true`, so the capture is cropped to the element's DOM rectangle — exactly, with no model-supplied coordinates, even if the element is scrolled off-screen. `take_screenshot` inspects pixels and returns only summary/detail text; `save_image` persists the visual. If the `@v` already has a transient crop/description, both actions reuse it; for URL-backed visuals, `save_image` first tries to save the original loaded image bytes, then falls back to the crop. With no ref, `take_screenshot` captures the full viewport for inspection. An unresolvable ref degrades to a full-viewport inspect rather than failing. Region presence and vision-derived descriptions are included in `briefHash`, so a page gaining, losing, or reclassifying a graphic re-prompts.
...
| `take_screenshot` | Inspect/read pixels | Top-level, `changesPage:false`, `idempotentRead`. Use for charts, diagrams, canvas, scans, CAPTCHAs, or image text when the agent needs to understand what pixels show. Crop-first — an optional `@v`/`@e`/`@t` ref clips to that node's bbox; no ref captures the viewport. Returns summary/detail only and does not create an asset. See `lib/screenshot.js` + `lib/visual-evidence.js`. |
| `save_image` | Promote a visual into report assets | Top-level, `changesPage:false`, `idempotentRead`. Use when the visual itself belongs in the final report. Requires an exact `@v`/`@e`/`@t` ref. For pre-enriched `@v` regions, reuses the transient crop/description; for URL-backed visuals, tries original loaded bytes before falling back to the crop. `intent` is the saved caption. Bytes → `runs/<id>/assets/`; saved path + description ride back as Observation detail. See `lib/screenshot.js` + `lib/visual-evidence.js`. |
...
| `save_file` | Download the bytes at a URL and save them | Top-level, `changesPage:false`. URL typically from `get_files` or a visible link (on-page visuals use `save_image` instead). Download is no-page-JS: `data:` decode → `Page.getResourceContent` (cached, exact, no CSP) → `Network.loadNetworkResource` (cold, CSP-limited). Image → vision summary; else metadata. Bytes → `assets/`; summary re-enters the event log. See `lib/savefile.js`. |
...
| `expand.provider` | `openai` | Provider for the Step-0 planning call. Defaults to the planner provider when unset. |
| `mode` | `"auto"` | Task shape: `auto`, `records`, or `research`. `auto` lets Step 0 classify the task. `records` forces a record contract (counts deliverables; never auto-stops the run); `research` suppresses the contract. |
| `expand.model` | `null` | Model for the Step-0 planning call. `null` inherits the planner model; this keeps task expansion interactive. |
...
| `vision.provider` | `openai` | Vision model provider for visual evidence, `take_screenshot`, `save_image`, and `save_file` on images — `openai`/`anthropic`/`gemini`/`ollama`. Independent of the planner `provider`. |
...
`node agent.js` is designed for another process to call. Progress, preflight guidance, and turn status are written to stderr. Stdout is exactly one compact JSON object with `ok`, `status`, `runId`, `task`, optional trusted `context`, the requested `mode`, Step-0 `taskType`, the run's `plan` of action (or `null`), `result`, final report content, and absolute paths to `report.md`, `report.html`, `saved.md`, `saved-index.md`, `assets/`, and the run logs. Callers should parse stdout and use the paths directly instead of reconstructing run locations from cwd.
...
Before the loop begins, the loop makes one **Step-0 call** (`models.expand`, enabled by default, inheriting the planner model unless configured otherwise). Its job is requirements extraction → translation: it **rewrites the bare operator task into one complete, self-contained instruction** — every requirement preserved, gaps filled, the whole thing strengthened for an LLM and phrased in the agent's canonical verbs (`navigate`/`click`/`save_record`/…) — plus an enumerated **`requirements`** checklist and a `taskType` classification. The `requirements` list captures every distinct thing the run must do or deliver, *including* imperative sub-steps that prose paraphrasing tends to drop (a task is often "collect X **and then** do Y" — both halves survive as discrete, independently checkable items). `mode:"auto"` lets Step 0 choose `records` or `research`; `mode:"records"` forces a record task; `mode:"research"` forces synthesis without a record contract. For record tasks, Step 0 also emits a small **record contract**: `recordName`, `target`, and a flat `fields` map (everything worth capturing for one record — no required/optional tiers, so a missing field never gates a save). The contract **counts** the deliverable records but never **stops** the run on its own — reaching the target just tells the model to stop collecting; it finishes by calling `done` once every requirement is satisfied. These are formed from the task and trusted `context` alone, **before any page is seen**. The rewritten task and requirements replace the operator's wording everywhere downstream — they are the agent's *one* statement of what to do. The task is threaded into three places so every stage shares one reading of it (the `requirements` checklist rides alongside it in the planner's cached prefix):
...
It is *not* a rigid script: the live page is always ground truth, and mid-run re-routing is the job of reflection, not a rewrite of the task. The record contract drives only the `save_record` ledger progress shown each turn; it does not make `save_text` evidence count as final output, and it is absent for research tasks. Because Step 0 is built only from trusted inputs (never page content), the rewritten task and requirements carry no prompt-injection surface. A failure here never breaks the run — `runArtifact.expandedTask` and `runArtifact.recordContract` stay `null`, `runArtifact.requirements` stays `[]`, and the loop falls back to the **raw operator task verbatim**, so the run proceeds exactly as if Step 0 were disabled. The expanded task, requirements, task type, and record contract are recorded on the Run artifact.
