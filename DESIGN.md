...
| `save_image` | Promote a vision-enriched visual into report assets | Top-level, `changesPage:false`. Requires a current `@v` ref. Reads the in-memory crop created by `visualEvidence`, writes it to `assets/`, and records image metadata in `saved-manifest.json`. No new screenshot or vision call. |
