...
**Visuals** (`regions`, surfaced as `@v` refs) are rendered graphics the accessibility tree can't describe in text — a `<canvas>`, an `<img>`, an `<svg>`, or a **cross-origin `<iframe>`** — that the model captures with `take_screenshot`. `extract.js` reads them straight from the DOMSnapshot (tag + attributes + layout) and emits one for each rendered graphic that is **not nested in a link/button** (those are control icons, already represented by the control). Two kinds fall out of the accessible-name check, both captured the same way:

- **Unnamed** — no `alt`/`aria-label`/`<title>` (a chart, map, scan, CAPTCHA, alt-less image, or cross-origin iframe). The tree is silent and the agent is blind, so it captures to **read** the content.
- **Named** — carries an accessible name (a logo, photo, or product shot with `alt`/`aria-label`). Its content *is* described, but a screenshot is how it gets **shown** in the final report. An `<img>` with an explicit empty `alt=""` is decorative and dropped.

This is a deterministic structural fact, not a heuristic about importance: we report that a graphic is painted here, and the planner decides whether to capture it. A **named** visual must also clear a size floor (`MIN_VISUAL_AREA`, ~80×80 — below it is an avatar/icon), is deduped by source, and is capped per brief (`NAMED_VISUAL_CAP`, biggest first) so a gallery or avatar list can't flood the listing. **Unnamed** graphics are never floored or capped — the model has no other way to see them.
...
Each visual gets a `@v` ref and a `lookup` entry (like `@e`/`@t`), and renders in the LLMView as an `[@vN]` line in reading order — with its name/title shown when it has one, and the note `take_screenshot @v to capture` (named) or `… to read` (unnamed). The ref is accepted **only by `take_screenshot`** — `click`/`type`/`selectText` reject `@v` at validation. When `take_screenshot` is given any ref (`@e`/`@t`/`@v`, all optional), the executor resolves it to that node's bbox and passes a `clip` to `Page.captureScreenshot` with `captureBeyondViewport:true`, so the capture is cropped to the element's DOM rectangle — exactly, with no model-supplied coordinates, even if the element is scrolled off-screen. With no ref, it captures the full viewport (the rare fallback, e.g. a whole-page shot or a CSS-`background-image` with no DOM node to clip). An unresolvable ref degrades to a full-viewport capture rather than failing. Region *presence* (role only, not bbox) is included in `briefHash`, so a page gaining or losing a graphic re-prompts.
